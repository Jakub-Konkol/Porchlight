...
class SpectralData():
    ...
    def SGSmooth(self, window, poly):
        ...
        self.spc = pd.DataFrame(savgol_filter(self.spc, window, poly, axis = 0), columns = self.spc.columns, index = self.spc.index)
    ...
    def SGDeriv(self, window, poly, order):
        ...
        self.spc = pd.DataFrame(savgol_filter(self.spc, window, poly, order, axis = 0), columns = self.spc.columns, index = self.spc.index)
    ...
    def minmax(self, min_val = 0, max_val = 1):
        self.spc = min_val + (self.spc.sub(self.spc.min(axis=0))) * (max_val - min_val) / (self.spc.max(axis=0) - self.spc.min(axis=0))
